# AGENT 01 — CVPR RMR-Count Research & Paper Master Specification

> **Role:** Research/paper agent.  
> **Target:** a CVPR-level/A-tier computer-vision paper, not a thesis document and not an engineering report.  
> **Method working name:** **RMR-Count — Regional Measure Reconciliation for Ultra-Light Crowd Counting**.  
> **Authority:** this file is the research/claim source of truth. `AGENT_02_RMR_IMPLEMENTATION_EXPERIMENT_MASTER.md` is the implementation source of truth. If implementation results contradict the hypotheses here, the results win; do **not** rescue the claim by silently adding modules.

## Non-negotiable agent rules

1. Start from **literature-supported limitation → verified gap → RQ → falsifiable hypothesis → method → experiment**.
2. Do not invent a field-level problem from a pathology specific to PS-FH/CMICF.
3. Do not claim novelty for prefix sums, summed-area tables, regional losses, local-count maps, inclusion–exclusion, generic iterative refinement, uncertainty, multi-scale fusion, generic deep unrolling, or generic forward/adjoint use.
4. The provisional novel core is only the **count-specific inference formulation**:
   \[
   Y^{(t)} ightarrow A Y^{(t)} ightarrow A Y^{(t)}-b
   ightarrow A^	op W(A Y^{(t)}-b) ightarrow Y^{(t+1)}.
   \]
5. Do not write “first”, “novel”, “SOTA”, or “new Pareto frontier” until the corresponding collision search / experiments support it.
6. The paper must be organized around **RQ1/RQ2**, not around a list of modules.
7. PS-FH `s=16,K=4` is an internal diagnostic/formulation baseline. It is not the field limitation.
8. The decisive operator-geometry test is **B5 exact-adjoint RMR vs B3b learned regional projector**, while **B3a local CNN** tests whether extra local capacity alone explains gains.
9. If B5 does not beat B2 and B3b under matched protocol, the central claim is killed or reframed. Do not add attention/MoE/teacher/distillation merely to save it.
10. Maintain a current literature collision ledger through submission.
...
# 30. Mandatory matched experiment matrix — registered V2

This matrix is the central scientific experiment. All variants must use the same dataset split, encoder, feature width, primary supervision, optimizer family, LR selected on validation only, augmentation, output stride, and checkpoint-selection rule. Report measured Params, MACs/FLOPs, latency, and memory rather than assuming “same cost.”

| ID | Variant | Regional information at inference | Correction geometry | Scientific purpose |
|---|---|---|---|---|
| **B0** | Direct-Y | none | none | carrier baseline |
| **B1** | RegionLoss | training only | none | does regional supervision alone solve it? |
| **B2** | RegionAux | predicted \(b_R\), auxiliary only | none | does multi-task regional prediction explain the gain? |
| **B3a** | LocalCNNRefine | none | learned local 3×3 scope | does extra local neural capacity explain the gain? |
| **B3b** | LearnedRegionalProject | same \(b_R\), same residuals, same region memberships | learned region-to-grid allocation | strongest geometry control |
| **B4** | RMR-T1 | same \(b_R\) | exact \(A^\top\), 1 iteration | one-step exact reconciliation |
| **B5** | RMR-T2 | same \(b_R\) | exact \(A^\top\)+bounded local preconditioner, 2 iterations | proposed canonical model if it survives |

### B3a fairness

B3a is deliberately local. It asks only whether adding a similarly small learned refinement block improves the fine carrier. It must **not** be presented as the sole “same-context” control because it lacks regional scope.

### B3b fairness — decisive geometry control

B3b must receive the same:
- regional estimates \(b_R\);
- regional residuals \(\delta_R=(AY-b)_R\);
- region set \(\mathcal R\);
- memberships \(p\in R\).

Instead of fixed uniform adjoint allocation, B3b learns a region-normalized allocation:
\[
\pi_{R,p}
=
...
Thus B3b has access to the **same regional scope and evidence** as RMR but is allowed to learn the projection geometry. It may be slower or larger; that is acceptable as long as compute is measured and it is not artificially handicapped.

The strongest causal comparison is:
\[
\boxed{B5\ \text{vs}\ B3b}
\]
for the operator-geometry claim, and:
\[
\boxed{B5\ \text{vs}\ B3a}
\]
for the “known operator vs extra local learned capacity” efficiency claim.

---

# 31. Mechanism ablations

## M1 — learned projection vs exact adjoint

Already registered as B3b vs B5.

Interpretation discipline:
- if B5 wins: exact count geometry is empirically useful;
- if B3b wins: do **not** conclude “linear operators fail because crowd distributions are nonlinear.” Possible causes include the chosen quadratic regional energy, weighting, correlated overlapping residuals, noise in \(b_R\), or suboptimal preconditioning.

## M2 — shuffled regional evidence

Shuffle \(b_R\) **within each scale family**. If performance does not degrade, the regional branch is not being used meaningfully.

## M3 — oracle regional evidence

Use exact GT regional counts only as a diagnostic:
\[
b_R=N_R^{gt}.
\]
Never report this as normal inference. It separates solver headroom from regional-estimator quality.

For scale \(s\):
\[
\Delta MAE_s^{oracle}
=
MAE(Y^{(0)})-MAE(Y_s^{oracle}).
\]

If oracle 16 px does not help, that scale is geometrically unnecessary. If oracle helps but predicted \(b_{16}\) hurts, the regional estimator—not the solver—is the bottleneck.
...
then:
\[
\{32\},\quad \{32,64\},\quad \{32,64,128\},\quad \{16,32,64,128\}.
\]

Do **not** start with 16 px by default.

## M5 — overlap

Compare non-overlapping versus 50% overlapping rectangles.
...
|---|---|
| **RQ1:** can exact count operators replace part of learned context under an ultra-light budget? | B0 vs B3a vs B5 + Params/MACs/latency |
| **RQ2:** does regional information add value at inference beyond training-only supervision? | B1/B2 vs B4/B5 |
| **Mechanism:** is exact \(A^\top\) geometry useful given the same regional evidence? | **B3b vs B5** |
| Regional-head bottleneck? | predicted vs oracle \(b_R\), scale-wise |
| Is \(b_R\) really used? | predicted vs shuffled \(b_R\) |

The main paper should be organized around these comparisons, not a sequential “add module” table.
...
The strongest kill-test is:

\[
B_{3b}\ \text{(learned regional projection)}
\quad\text{vs}\quad
B_5\ \text{(exact-adjoint RMR)}.
\]

If B3b consistently matches or beats B5 under matched evidence and fair reporting, the claim that exact adjoint geometry is a superior inference prior is not supported.

B3a is a secondary control: if a purely local CNN matches B5 with similar or lower real compute, then RMR's claimed efficiency advantage is also weak.
...
# 33A. Critical implementation traps — paper-level interpretation policy

These are not optional engineering notes; they affect causal validity.

### Trap 1 — unfair B3 control

Do not claim fairness merely by matching parameter count. B3a and B3b answer different questions. B3b must have the same regional evidence and membership scope as RMR.

### Trap 2 — unrolled instability

The risk is not that \(A^\top\) is intrinsically unstable. The magnitude of
\[
A^\top W(AY-b)
\]
depends on overlap, scale, count magnitude, \(b_R\) noise, coverage, and step size.

Use:
\[
r=
\frac{A^\top W(AY-b)}
{A^\top W\mathbf1+\epsilon},
\]
residual clipping, bounded \(\eta_t\), and solver warm-up/ramp.
...
- fraction \(z<-10\);
- regional error before and after each iteration.

Because:
\[
Y=\operatorname{softplus}(z),
\]
\(Y\) itself cannot become negative; the actual risk is \(z\ll0\) and softplus/sigmoid saturation.

### Trap 3 — regional head poisoning

Do not compare 16-px regional MAE directly with whole-image MAE. Report:
\[
NMAE_R=
\frac{|b_R-N_R|}
{\max(N_R,1)}
\]
and stratify by true region count:
\[
0,\ 1,\ 2\text{–}4,\ 5\text{–}9,\ 10+.
\]

Start with \(\{32,64,128\}\); add 16 px only after oracle/predicted diagnostics.

### Trap 4 — overinterpreting B3b wins

If learned projection beats \(A^\top\), it kills or weakens the exact-geometry claim. It does **not** by itself prove that “linear operators fail in dense counting.” Re-evaluate the energy, weights, overlap correlation, and regional-head noise before making a broader scientific claim.
...
The Research Questions would have been answered negatively, and the correct scientific action is to reformulate the paper framing.

---

# 62. Bottom line

The clean CVPR paper framing is not:
...
That framing comes from the intersection of documented limitations in the field, has a clear prior-art boundary, and can be falsified with matched experiments.

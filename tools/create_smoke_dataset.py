...
def create_synthetic_crowd_dataset(root: str = "./data/synthetic_crowd", num_train: int = 16, num_val: int = 8, crop_size: int = 448):
    """Create synthetic dataset mimicking ShanghaiTech format with empty, sparse, and dense images."""
    for split, count in [("train_data", num_train), ("test_data", num_val)]:
        ...
        for i in range(count):
            ...
            if num_pts > 0:
                pts = np.random.uniform(10, crop_size - 10, size=(num_pts, 2)).astype(np.float32)
            else:
            ...
            location_dict[0, 0] = (pts, )
            image_info[0, 0] = location_dict
            
            sio.savemat(mat_path, {"image_info": image_info, "annPoints": pts, "number": num_pts})

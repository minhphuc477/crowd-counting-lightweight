...
def compute_nb_interval_coverage(
    ...
    For region R with predicted mean mu_R and dispersion r_R, the NB distribution has:
      n = r_R
      p = r_R / (r_R + mu_R)
    Confidence intervals at level alpha in (0, 1) are defined by:
      q_low = nbinom.ppf((1 - alpha) / 2, n, p)
      q_high = nbinom.ppf(1 - (1 - alpha) / 2, n, p)

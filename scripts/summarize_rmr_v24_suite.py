"""Summarize RMR-v24 benchmark results across all 6 matrix runs."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Summarize RMR-v24 benchmark and ablation results.")
    parser.add_argument("--runs-dir", default="runs/sha_a", help="Directory containing run directories")
    parser.add_argument("--output-md", default="runs/sha_a/rmr_v24_benchmark_summary.md", help="Path to output Markdown")
    parser.add_argument("--output-csv", default="runs/sha_a/rmr_v24_benchmark_summary.csv", help="Path to output CSV")
    ...
    canonical_order = [
        ("rmr_v24_canonical", "RMR-v24 Canonical (ABB + Micro-Perspective + Gated Diffusion)"),
        ("rmr_v24_ablation_no_abb", "Ablation 1: No Alternating BB (Fixed omega=1.0)"),
        ("rmr_v24_ablation_no_perspective", "Ablation 2: No Micro-Perspective Elevation"),
        ("rmr_v24_ablation_no_density_gated_diffusion", "Ablation 3: No Density-Gated Diffusion"),
        ("rmr_v24_ablation_no_curvature", "Ablation 4: No Curvature Regularization"),
        ("rmr_v24_control_no_solver", "Control: Direct Feedforward (No SIRT Solver)"),
    ]
    ...
    for run_id, desc in canonical_order:
        ...
        if val_summary.is_file():
            try:
                ...
                if run_id == "rmr_v24_canonical" and "mae" in data:
    ...
    # Markdown format
    md_lines = [
        "# RMR-v24 Benchmark & Ablation Study Summary",
        "",

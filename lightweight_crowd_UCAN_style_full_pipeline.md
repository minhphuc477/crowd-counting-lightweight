# Lightweight Crowd Counting — UCAN-style Bottleneck Research Plan + Full Code

## 0. Purpose

This document implements the **pre-model diagnostic stage** before any final architecture is invented.
The current goal is to find a bottleneck as concrete as UCAN's rank-collapse story:

- **G-K / D-K:** lightweight inter-person separability collapse.
- **G-R / D-R:** sampling-phase / ±1–2 px shift instability.
- **G-L / D-L:** normalized effective-rank collapse.
- **D1 zoom control:** distinguish resolution loss from representation failure.

The carrier model is intentionally simple. It is **not** the paper contribution.
A final operator is designed only after one bottleneck survives the falsification gates.

---

# 1. Research questions

### RQ-K — inter-person separability
At matched head scale and input resolution, does a compact encoder merge neighboring-person evidence earlier than a stronger encoder, even when early-layer evidence remains separable?

### RQ-R — sampling phase
Can a semantic-preserving ±1/±2 pixel translation cause materially larger count/map/feature changes in compact counters, especially for tiny/dense heads?

### RQ-L — effective representation rank
Does a compact single-path encoder use a substantially smaller independent feature subspace than its nominal channel width in difficult crowd regions, and does this collapse increase through depth?

---

# 2. Falsification gates

## G-K GO
All of the following should hold:

1. Zooming does **not** explain most of the failure.
2. Error increases as normalized nearest-neighbor spacing decreases after controlling head scale.
3. Compact model degrades at a materially larger spacing / steeper slope than the strong model.
4. Early compact features retain two-person evidence, but later compact features become more merged.

Kill G-K if both compact and strong models fail at the same spacing, or if image resolution alone explains the effect.

## G-R GO
Require:

1. ±1/±2 px shifts create higher count variance in compact models than strong models.
2. The effect is concentrated on tiny/dense examples, not borders only.
3. A clear jump appears at a particular downsampling stage.
4. Shift instability correlates with real counting failures.

Kill G-R if the effect is only border/padding noise or all capacities are equally unstable.

## G-L GO
Require:

1. Normalized effective rank falls through depth more strongly in compact models.
2. The effect is strongest in difficult/close-spacing crowd examples.
3. Rank loss predicts count error beyond simple channel count and image difficulty.

Kill G-L if rank only tracks nominal width or generic image complexity.
...
Stem, stride 2
 ↓
Depthwise stage, stride 4   → s4 diagnostic feature
 ↓
Depthwise stage, stride 8   → s8 diagnostic feature
 ↓
Depthwise stage, stride 16  → s16 diagnostic feature
 ↓
minimal additive decoder
 ↓
Softplus positive mass map at stride 4
 ↓
sum(map) = predicted count
```

Capacity ladder used by the code:

| Width | Approx. params from self-test |
|---:|---:|
| 0.35 | 47,705 |
| 0.50 | 66,929 |
| 1.00 | 181,993 |
...
The same architecture family is used across widths to avoid architecture-family confounding.

---

# 4. Training objective

The diagnostic carrier uses a **Gaussian-free local block-count objective**:

1. Point annotations are rasterized as discrete count impulses at the stride-4 output grid.
2. Prediction and GT are summed into local blocks.
3. Smooth-L1 local block-count loss is used.
4. A global count Smooth-L1 term is added.

This deliberately avoids making the first diagnostic depend on Gaussian density smoothing.
Later publication experiments must still include external point/OT/block baselines.
...
```csv
image,points,sizes,split
/kaggle/input/data/img001.jpg,/kaggle/input/data/ann001.npy,/kaggle/input/data/size001.npy,train
/kaggle/input/data/img002.jpg,/kaggle/input/data/ann002.npy,/kaggle/input/data/size002.npy,val
/kaggle/input/data/img003.jpg,/kaggle/input/data/ann003.npy,/kaggle/input/data/size003.npy,test
```

`points` is `N×2 [x,y]`.

`sizes` is optional:
- `N×1`: head diameter, or
- `N×2`: width/height; code converts to `sqrt(w*h)`.

For the **final D-K claim**, use real head-size proxies from box/size annotations. If `sizes` is absent, the script only reports raw pixel spacing and explicitly warns that normalized-spacing evidence is not sufficient.

Supported point files: `.npy`, `.npz`, `.txt`, `.json`, `.mat`.
...
```bash
pip install torch pandas numpy pillow scipy
```

On Kaggle, PyTorch is already installed in most GPU runtimes.

---

# 7. Full runnable code

Save the following as `d0_lightweight_crowd_pipeline.py`.
...
```bash
python d0_lightweight_crowd_pipeline.py selftest
```

The supplied code was syntax-checked and self-tested when this file was generated.

## 8.2 Train the capacity ladder

Tiny:

```bash
python d0_lightweight_crowd_pipeline.py train   --manifest /kaggle/working/manifest.csv   --width 0.35   --crop 512   --batch-size 8   --epochs 300   --lr 3e-4   --amp   --out /kaggle/working/carrier_w035.pt
```

Small:

```bash
python d0_lightweight_crowd_pipeline.py train   --manifest /kaggle/working/manifest.csv   --width 0.50 --crop 512 --batch-size 8 --epochs 300 --lr 3e-4 --amp   --out /kaggle/working/carrier_w050.pt
```

Medium:

```bash
python d0_lightweight_crowd_pipeline.py train   --manifest /kaggle/working/manifest.csv   --width 1.0 --crop 512 --batch-size 6 --epochs 300 --lr 3e-4 --amp   --out /kaggle/working/carrier_w100.pt
```

Strong same-family control:

```bash
python d0_lightweight_crowd_pipeline.py train   --manifest /kaggle/working/manifest.csv   --width 1.5 --crop 512 --batch-size 4 --epochs 300 --lr 3e-4 --amp   --out /kaggle/working/carrier_w150.pt
```

Use identical data split, objective, crop policy and optimization when comparing widths.

---

# 9. D-R — shift / sampling-phase screen

Run this **first** because it is cheap and requires no retraining:

```bash
python d0_lightweight_crowd_pipeline.py dr   --manifest /kaggle/working/manifest.csv   --split test   --models tiny=/kaggle/working/carrier_w035.pt,strong=/kaggle/working/carrier_w150.pt   --max-shift 2   --out /kaggle/working/dr_shift_raw.csv
```
...
- `ShiftCountStd`
- `ShiftCountRange`
- inverse-aligned mass-map L1
- stagewise `s4/s8/s16` feature cosine consistency

Interpretation:

```text
large compact-model instability
        +
small strong-model instability
        +
stage-specific jump
        +
tiny/dense concentration
        ↓
G-R survives
```

Do **not** immediately add BlurPool/APS/TIPS. Those are controls/prior art, not the contribution.

---

# 10. D1 — zoom recovery control

```bash
python d0_lightweight_crowd_pipeline.py zoom   --manifest /kaggle/working/manifest.csv   --split test   --models tiny=/kaggle/working/carrier_w035.pt,strong=/kaggle/working/carrier_w150.pt   --scales 1,2,4   --out /kaggle/working/d1_zoom.csv
```

If 2×/4× zoom removes most compact-model failures, the dominant bottleneck is resolution/information loss rather than a new representation mechanism.

---

# 11. D-K — inter-person separability screen

```bash
python d0_lightweight_crowd_pipeline.py dk   --manifest /kaggle/working/manifest.csv   --prototype-split train   --split test   --models tiny=/kaggle/working/carrier_w035.pt,strong=/kaggle/working/carrier_w150.pt   --n-bins 5   --out /kaggle/working/dk_pairs.csv
```

The code computes:

- nearest-neighbor spacing,
- normalized spacing when head sizes exist,
- local signed count error,
- stagewise head-prototype response,
- midpoint-to-head **merge ratio** at `s4/s8/s16`,
- pair descriptor cosine as a secondary diagnostic.

The primary spatial merge statistic is:

```text
merge_ratio = headness(midpoint) / mean(headness(person_1), headness(person_2))
```

A rising merge ratio through depth means the valley between two people is disappearing.

The strongest G-K pattern is:

```text
                compact              strong
s4              separated            separated
s8              partly merged        separated
s16             strongly merged      still separated
```

while local count bias becomes increasingly negative at small normalized spacing.

---

# 12. D-L — effective-rank screen

```bash
python d0_lightweight_crowd_pipeline.py dl   --manifest /kaggle/working/manifest.csv   --split test   --models tiny=/kaggle/working/carrier_w035.pt,small=/kaggle/working/carrier_w050.pt,medium=/kaggle/working/carrier_w100.pt,strong=/kaggle/working/carrier_w150.pt   --n-bins 4   --min-bin-n 64   --out /kaggle/working/dl_effective_rank.csv
```

For feature matrix `X ∈ R^(N×C)`, the script reports:

### Entropy effective rank

`exp(-Σ p_i log p_i)` where `p_i = σ_i² / Σσ_j²`.

### Participation rank

`(Σσ_i²)² / Σσ_i⁴`.

The primary normalization is by **maximum feasible rank `min(N,C)`**, not blindly by `C`.

Desired evidence:

```text
hard / close-spacing heads:
  compact s4  -> high normalized rank
  compact s8  -> lower
  compact s16 -> collapse

  strong s4/s8/s16 -> substantially better retention
```

---

# 13. Minimum experiment order

Do not train a proposed module before these gates.

```text
1. Train same-family capacity ladder
2. D-R shift screen
3. D1 zoom control
4. D-K proximity + early/late separability
5. D-L effective rank
6. Decide which bottleneck survives
7. Search novelty around ONLY that failure
8. Design one minimal operator
9. Parameter/FLOP-matched ablation
10. Full SHA/QNRF/NWPU/JHU campaign
```

---

# 14. How the final model should be chosen

### If only G-R survives
Design a **phase/evidence-preserving transition** around the measured failing downsampling stage.
Do not claim generic anti-aliasing as novelty.

### If only G-K survives
Design a **minimal inter-person evidence-preserving transition**.
Do not use dynamic receptive field, MoE, masks, body parsing, or generic head/background contrastive learning.

### If G-K + G-L survive
This is the strongest UCAN-like story:

> compact transition progressively compresses neighboring-person evidence into a low-dimensional/merged representation.

Then one minimal operator should directly preserve the measured separability/rank property.

### If G-R + G-K + G-L survive
Potential central claim:

> **Aggressive lightweight feature transitions are not merely lower-capacity; they become sampling-phase sensitive and collapse independent neighboring-person evidence during depth progression.**

This is strong enough to justify a dedicated lightweight crowd operator if novelty search finds no equivalent mechanism.

### If all fail
Stop. Do not rescue the hypothesis with modules. Pivot to the same-scene `±k person` frozen-counter audit already preserved in the research ledger.

---

# 15. Publication experiment after a bottleneck GO

Only after one diagnostic survives:
...
### Required ablations
- carrier baseline
- proposed operator at s4→s8 only
- proposed operator at s8→s16 only
- both transitions
- parameter-matched ordinary convolution
- FLOP-matched wider carrier
- strong same-family control

### Required mechanism evidence
- the exact D-R/D-K/D-L metric that motivated the operator
- density/head-size/proximity bins
- signed error, not only MAE
- ≥3 seeds for final claims

### Lightweight evidence
- parameters
- MACs/FLOPs
- batch-1 latency/FPS on the chosen GPU

Efficiency is supporting evidence, not the novelty by itself.

---

# 16. Important exclusions from the literature audit

Do **not** turn the final model into a renamed version of:

- density MoE / routing,
- dynamic receptive fields / dense-region smaller RF,
- local/global multibranch fusion,
- generic low-level skip/detail preservation,
- Gaussian-kernel narrowing,
- new OT/counting loss,
- body/neck/shoulder parser,
- visible-part auxiliary supervision,
- mask/exclusivity contrastive learning,
- generic feature-discrimination attention,
- generic effective-rank/channel-expansion block without D-L evidence,
- generic anti-aliasing/BlurPool without D-R evidence,
- ZIP/background-only loss without a new capacity-specific gradient result.

The measured bottleneck must determine the model, not the other way around.

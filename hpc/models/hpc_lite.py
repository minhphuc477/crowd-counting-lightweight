...
class HPCLite(nn.Module):
    """HPC-Lite crowd counter with MobileNetV4 backbone (~0.26M deploy parameters).

    Achieved MAE 77.93 on master branch.
    Architecture:
    ...
    def switch_to_deploy(self):
        """Fuse structural reparameterization branches into single Conv2d for inference."""
    ...
    @torch.no_grad()
    def predict(self, x: torch.Tensor, pad_multiple: int = 32) -> Tuple[torch.Tensor, torch.Tensor]:
        if x.ndim != 4:
            raise ValueError(f"Expected 4D input, got {tuple(x.shape)}")
        _, _, h, w = x.shape
        pad_h = (pad_multiple - (h % pad_multiple)) % pad_multiple
